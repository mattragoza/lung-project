# preprocessing/mask_processing.py
...
def preprocess_binary_mask(
    ...
    if do_filter:
        mask = filter_binary_mask(mask, **filter_kws)
...
def filter_binary_mask(mask: np.ndarray, **kwargs) -> np.ndarray:
    utils.log(f'Filtering foreground (removing blobs)')
    mask = filter_connected_components(mask != 0, **kwargs)
