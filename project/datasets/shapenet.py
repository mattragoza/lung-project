...
DEPENDENCY_GRAPH = {
    'binary_mask':   [],
    'surface_mesh':  [],
    'region_map':    ['binary_mask'],
    'volume_mesh':   ['region_map'],
    'material_map':  ['region_map'],
    'density_field': ['material_map'],
    'elastic_field': ['material_map'],
    'poisson_field': ['material_map'],
    'input_image':   ['material_map'],
    'material_mesh': ['material_map', 'volume_mesh'],
...
class ShapeNetDataset(base.Dataset):
    ...
    def load_metadata(self):
        ...
        self._metadata.set_index(self.ID_COLUMN, inplace=True)
    ...
    def examples(
        ...
        for sid in subject_list:
            ...
            meta = {'raw': dict(m)}
            if parse_metadata:
                meta['category'] = _parse_category(m['category'])
                meta['dims'] = _parse_aligned_dims(m['aligned.dims'])
                meta['unit'] = _resolve_unit(m['unit'], default_unit, unit_policy)

            if variant: # assets generated by preprocessing
                s, v = sid, variant
                paths['binary_mask']  = self.derived_path(sid, v, 'mask', names['binary_mask'])
                paths['region_map']   = self.derived_path(sid, v, 'mask', names['region_map'])
                paths['material_map'] = self.derived_path(sid, v, 'mask', names['material_map'])

                paths['surface_mesh']  = self.derived_path(sid, v, 'mesh', names['surface_mesh'])
                paths['volume_mesh']   = self.derived_path(sid, v, 'mesh', names['volume_mesh'])
                paths['material_mesh'] = self.derived_path(sid, v, 'mesh', names['material_mesh'])
                paths['simulate_mesh'] = self.derived_path(sid, v, 'mesh', names['simulate_mesh'])
                paths['interp_mesh']   = self.derived_path(sid, v, 'mesh', names['interp_mesh'])

                paths['input_image']   = self.derived_path(sid, v, 'image', names['input_image'])
