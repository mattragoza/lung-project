...
def structure_tensor(img, sigma):
    ...
    for i in range(C):
        for j in range(i, C):
            struct.append(filters.smooth(
                (img[:, i, ...] * img[:, j, ...]).unsqueeze(1), sigma
            ))
...
def foerstner_kpts(img, mask, sigma=1.4, d=9, thresh=1e-8):
    ...
    grad = torch.cat([
        filters.filter1D(img, filt, 0),
        filters.filter1D(img, filt, 1),
        filters.filter1D(img, filt, 2)
    ], dim=1)
    ...
    maxfeat = F.max_pool3d(F.pad(
        distinctiveness, (pad2, pad1, pad2, pad1, pad2, pad1)
    ), d, stride=1)
    ...
    mask_eroded = (
        1 - F.conv3d(
            1 - mask.float(), structure_element.unsqueeze(0).unsqueeze(0),
            padding=1
        ).clamp_(0, 1)
    ).bool()
